...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`user_comment` ;
...
CREATE TABLE IF NOT EXISTS `mydb`.`user_comment` (
  ...
  INDEX `fk_user_message_user_message1_idx` (`fk_comment_id_parent` ASC),
  ...
  CONSTRAINT `fk_user_message_user_message1`
    FOREIGN KEY (`fk_comment_id_parent`)
    REFERENCES `mydb`.`user_comment` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`layer_comment` (
  ...
  INDEX `fk_layer_comment_layer_comment1_idx` (`fk_comment_id_parent` ASC),
